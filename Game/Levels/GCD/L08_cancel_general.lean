...
If $a \cdot c \equiv b \cdot c \pmod m$, `gdc(c, m) = d` with $d \neq 0$, $c = c_1 \cdot d$ and $m = m_1 \cdot d$, then $a \equiv b \pmod{m_1}$.

**Intuition:**
This is the fully general version of "dividing" a congruence: you can always cancel a factor $c$ out of a congruence, as long as you also divide the modulus by whatever $c$ and $m$ have in common, $d = \gcd(c,m)$. The coprime case (Level 6) is just the special case $d = 1$, where the modulus doesn't need to shrink at all.
-/
TheoremDoc mod_cancel_general as "mod_cancel_general" in "GCD"

/-- The fully general cancellation law for congruences. -/
Statement mod_cancel_general (a b c m d c1 m1 : ℤ) (hd : d ≠ 0) (h1 : (a * c) ≡ (b * c) (mod m)) (h2 : gcd(c, m)= d) (hc : c = c1 * d) (hm : m = m1 * d) : a ≡ b (mod m1) := by
  have hcoprime : gcd(c1, m1)= 1 := gcd_div_coprime c m d c1 m1 hd h2 hc hm
  unfold ModEq at h1
  obtain ⟨k, hk⟩ := h1
  rw [hc, hm] at hk
